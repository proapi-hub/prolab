...
- 画布项目 JSON：`localForage`，数据库名 `prolab`，storeName `app_state`，key 为 `prolab:canvas_store`。
- 我的素材 JSON：`localForage`，数据库名 `prolab`，storeName `app_state`，key 为 `prolab:asset_store`。
- 图片 Blob：单独存到 `localForage` 实例，数据库名 `prolab`，storeName `image_files`。
- 视频等媒体 Blob：单独存到 `localForage` 实例，数据库名 `prolab`，storeName `media_files`。
